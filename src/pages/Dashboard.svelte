...
{:else if currentPage === "dashboard"}
  ...
  <!-- CAMBIO 1: Eliminamos max-w-[1920px]. Ahora es w-full con padding adaptativo -->
  <main class="w-full px-4 md:px-8 lg:px-12 2xl:px-20 py-6 md:py-12 2xl:py-16">
    
    <!-- Hero Section -->
    <div
      class="glass w-full rounded-xl p-6 md:p-8 2xl:p-16 mb-8 md:mb-12 gradient-dark text-white flex flex-col md:flex-row items-center justify-between gap-8 2xl:gap-16"
    >
      <div class="flex-1 w-full">
        <h2 class="text-3xl md:text-5xl 2xl:text-6xl font-bold mb-4 2xl:mb-8 text-white">
          Hola, {$currentUser.name}! 👋
        </h2>
        <!-- Permitimos que el texto respire ocupando más espacio -->
        <p class="text-lg md:text-xl 2xl:text-3xl text-white/90 mb-8 2xl:mb-12 max-w-none lg:w-4/5 leading-relaxed">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 2xl:gap-8">
          <div class="bg-white/10 backdrop-blur p-6 2xl:p-8 rounded-xl">
            <div class="text-4xl 2xl:text-5xl font-bold mb-2">6+</div>
            <p class="text-base 2xl:text-xl text-white/90">Herramientas disponibles</p>
          </div>
          <div class="bg-white/10 backdrop-blur p-6 2xl:p-8 rounded-xl">
            <div class="text-4xl 2xl:text-5xl font-bold mb-2">2+</div>
            <p class="text-base 2xl:text-xl text-white/90">Tutoriales interactivos</p>
          </div>
          <div class="bg-white/10 backdrop-blur p-6 2xl:p-8 rounded-xl">
            <div class="text-4xl 2xl:text-5xl font-bold mb-2">∞</div>
            <p class="text-base 2xl:text-xl text-white/90">Posibilidades de aprendizaje</p>
          </div>
        </div>
      </div>
      
      <div class="hidden md:flex w-full md:w-1/3 xl:w-1/4 justify-center">
        <img
          src="/src/assets/hero.png"
          alt="Hero Illustration"
          class="w-full max-w-[500px] 2xl:max-w-[700px] h-auto object-contain opacity-90 drop-shadow-2xl brightness-110"
        />
      </div>
    </div>

    <!-- Sección Paso a Paso (Tutoriales) -->
    <div class="mb-12 2xl:mb-20 w-full">
      <div class="flex items-center gap-3 mb-6 2xl:mb-10">
        <span class="text-3xl 2xl:text-4xl">🚀</span>
        <h2 class="text-2xl 2xl:text-4xl font-bold text-white">Paso a paso</h2>
      </div>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 2xl:gap-10">
        {#each tutorials as item}
          <div
            class="bg-white border border-gray-200 p-6 2xl:p-10 rounded-xl shadow-sm hover:shadow-md transition-shadow h-full"
          >
            <p class="text-xs 2xl:text-sm font-bold tracking-[0.2em] text-[#c41e3a] mb-3 2xl:mb-4">
              PASO {item.step}
            </p>
            <h3 class="text-lg 2xl:text-2xl font-bold text-gray-900 mb-3">{item.title}</h3>
            <p class="text-sm 2xl:text-lg text-gray-700 leading-relaxed">{item.text}</p>
          </div>
        {/each}
      </div>
    </div>
    ...
    <div class="mb-10 2xl:mb-16 w-full">
      <div class="flex items-center gap-3 mb-6 2xl:mb-8">
        <span class="text-3xl 2xl:text-4xl">🛠️</span>
        <h2 class="text-2xl 2xl:text-4xl font-bold text-white">Herramientas</h2>
      </div>
      <div class="mb-6 2xl:mb-10">
        <input
          ...
          class="w-full px-6 py-4 2xl:px-8 2xl:py-6 2xl:text-xl border border-gray-400 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#c41e3a] transition-all shadow-sm text-gray-900"
        />
      </div>

      <div class="flex flex-wrap gap-3 2xl:gap-5">
        <button
          on:click={() => ($selectedCategory = null)}
          class="px-5 py-2.5 2xl:px-8 2xl:py-4 2xl:text-lg rounded-full transition-all font-medium {$selectedCategory === null
        ...
        {#each toolCategories as category}
          <button
            ...
            class="px-5 py-2.5 2xl:px-8 2xl:py-4 2xl:text-lg rounded-full transition-all flex items-center gap-2 font-medium {$selectedCategory === category.id
              ? 'bg-[#c41e3a] text-white'
              : 'bg-gray-200 text-gray-800 hover:bg-gray-300'}"
          >
    ...
    <div
      class="w-full grid gap-6 2xl:gap-10"
...
{:else if currentPage === "tools" && selectedTool}
  ...
  <main class="w-full px-4 md:px-8 lg:px-12 2xl:px-20 mx-auto py-12 2xl:py-20">
    <div class="glass w-full rounded-xl 2xl:rounded-2xl p-8 lg:p-12 2xl:p-20 mb-8 bg-white/95 shadow-xl">
      <div class="grid md:grid-cols-3 gap-8 2xl:gap-16">
        <div>
          <div class="text-7xl 2xl:text-[8rem] mb-6 2xl:mb-10">{selectedTool.icon}</div>
          <div class="space-y-6 2xl:space-y-10">
            <div>
              <p class="text-sm 2xl:text-xl font-bold text-gray-800 mb-2">Dificultad</p>
              <span
                class="inline-block px-4 py-2 2xl:px-6 2xl:py-3 rounded-full text-sm 2xl:text-lg font-bold {selectedTool.difficulty === 'beginner'
            ...
            <div>
              <p class="text-sm 2xl:text-xl font-bold text-gray-800 mb-2">Costo</p>
              <p class="font-bold text-xl 2xl:text-3xl text-gray-900">
                {selectedTool.isFree ? "🆓 Gratis" : "💵 Pago"}
              </p>
            </div>
            <div>
              <p class="text-sm 2xl:text-xl font-bold text-gray-800 mb-2">Calificación</p>
              <div class="flex items-center gap-2">
                <div class="flex text-xl 2xl:text-3xl">
                  {#each Array(5) as _, i}
                    <span
                      class="{i < Math.round(selectedTool.rating) ? '⭐ text-[#ffb81c]' : '☆ text-gray-400'}"
                    ></span>
                  {/each}
                </div>
                <span class="text-sm 2xl:text-xl text-gray-800 font-medium">({selectedTool.reviews})</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="md:col-span-2 flex flex-col justify-center">
          <h2 class="text-4xl 2xl:text-6xl font-bold text-gray-900 mb-8 2xl:mb-12">
            {selectedTool.name}
          </h2>

          <div class="mb-8 2xl:mb-12">
            <h3 class="text-xl 2xl:text-3xl font-bold text-gray-900 mb-4 2xl:mb-6">Casos de Uso</h3>
            <ul class="space-y-3 2xl:space-y-5">
              {#each selectedTool.useCases as useCase}
                <li class="flex items-start gap-3 2xl:gap-4">
                  <span class="text-[#c41e3a] font-bold mt-1 2xl:text-2xl">✓</span>
                  <span class="text-gray-800 2xl:text-2xl leading-relaxed">{useCase}</span>
                </li>
              {/each}
            </ul>
          </div>
          ...
          <div class="mb-10 2xl:mb-16">
            <h3 class="text-xl 2xl:text-3xl font-bold text-gray-900 mb-4 2xl:mb-6">Etiquetas</h3>
...
{#if isTutorialOpen}
  ...
  >
    ...
    >
      ...
      >
        <div class="px-6 py-4 md:px-7 md:py-6 2xl:px-10 2xl:py-8 border-b border-white/10 flex-none">
          <div class="flex items-center justify-between mb-3 md:mb-5">
            <div>
              <p class="text-xs 2xl:text-sm uppercase tracking-[0.2em] text-cyan-200 font-bold mb-1">
            ...
            >
              <svg class="w-6 h-6 2xl:w-8 2xl:h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="space-y-2">
            ...
            <div class="h-1.5 2xl:h-2 w-full bg-white/10 rounded-full overflow-hidden">
              <div class="h-full bg-[#c41e3a] transition-all duration-500" style="width: {tutorialProgress}%"></div>
            </div>
          </div>
        </div>

        <div class="flex-1 overflow-y-auto px-6 py-4 md:px-7 md:py-5 2xl:px-10 2xl:py-8 custom-scrollbar space-y-4 md:space-y-6">
          <h4 class="text-2xl md:text-3xl 2xl:text-4xl font-black leading-tight mb-4">
            <span class="text-cyan-400">Paso {tutorialStep + 1}:</span>
            <span class="text-white">{currentTutorialSteps[tutorialStep].title}</span>
          </h4>

          <p class="text-base md:text-lg 2xl:text-2xl leading-snug text-white/80">
            {currentTutorialSteps[tutorialStep].description}
          </p>

          {#if currentTutorialSteps[tutorialStep].tip}
            <div class="bg-cyan-400/10 border border-cyan-400/30 p-4 md:p-6 2xl:p-8 rounded-2xl mt-6 md:mt-8">
              <p class="text-sm md:text-base 2xl:text-xl font-medium leading-snug text-cyan-100 flex gap-2 md:gap-3">
        ...
        <div class="p-4 md:p-6 2xl:p-8 bg-slate-950/50 border-t border-white/10 flex-none flex gap-2 md:gap-4">
          <button
            on:click={prevStep}
      ...
      <!-- Imagen Interactiva -->
      <div class="flex-1 bg-black relative flex items-center justify-center p-0 overflow-hidden">
        ...
        <div class="w-full h-full relative p-2 md:p-4 2xl:p-8">
          <div class="relative w-full h-full bg-slate-800 rounded-xl 2xl:rounded-2xl overflow-hidden shadow-2xl border border-white/5">
            ...
            {:else}
              <div class="w-full h-full flex flex-col items-center justify-center text-white/20">
                ...
                <span class="text-2xl 2xl:text-4xl font-bold">Sin previsualización</span>
              </div>
            {/if}
          </div>
        </div>
